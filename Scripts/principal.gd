...
func generarCartas() -> void:
	...
	for i in cartasEnemigo:
		...
		if tiempoCarta > 0:
			cartas_enemigo.get_node("Carta"+str(i)+"/congeladoF/Label").text = str(tiempoCarta)
...
func condicionJuego():
	if VARIABLES.saludJugador <= 0:
		...
		condicion_juego.get_node("jugadorMuerto").visible = true
		timer.stop()
		timer_2.stop()
		timer_3.stop()
		visibleLucha(false)
	...
	if VARIABLES.saludEnemigo <= 0:
		condicion_juego.visible = true
		condicion_juego.get_node("enemigoMuerto").visible = true
		timer.stop()
		timer_2.stop()
		timer_3.stop()
		visibleLucha(false)
